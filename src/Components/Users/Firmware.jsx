...
const Firmware = () => {
  ...
  useEffect(() => {
    const getAllBrokers = async () => {
      try {
        ...
        const res = await axios.get("http://13.201.135.43:5000/api/brokers", {
  ...
  const fetchVersions = async (brokerIp) => {
    try {
      const response = await fetch(`http://13.201.135.43:5000/api/get-all-versions?ip=${brokerIp}`);
      if (!response.ok) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const response = await fetch("http://13.201.135.43:5000/api/upload", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`http://13.201.135.43:5000/api/delete/${encodeURIComponent(filename)}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handlePublish = async (index) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://13.201.135.43:5000/api/publish`,
